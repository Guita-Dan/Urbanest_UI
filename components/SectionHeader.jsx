...
const SectionHeader = ({ title, description, buttonTitle }) => {
    return (
        <Container className="flex flex-col py-12 px-8 lg:px-0 gap-4">
            <div className="starIcon">
                <StarIcon />
            </div>
            <h2 className="text-2xl font-semibold leading-normal xl:text-3xl xl:leading-normal 2xl:leading-normal 2xl:text-4xl  ">{title}</h2>
            <div className="flex justify-center md:justify-between items-end flex-col md:flex-row gap-4">
                ...
                {buttonTitle &&
                    <Link
                        ...
                        target="_blank"
                        rel="noopener">
                        <span className="block  py-4 px-5 border border-solid border-[#262626] rounded-lg bg-[#1A1A1A] hover:bg-[#1E1E1E] hover:border-[#1A1A1A]">{buttonTitle}</span>
                    </Link>
                }
            </div>
